...
{
    DPRINTF(HWPrefetch, "Current round"
        "is %d, bestScore is %d\n", round, bestScore);
        ...
        std::vector<AddrPriority> &addresses)
{
    ...
    if (issuePrefetchRequests) {
        ...
        if (samePage(prefetch_addr, addr)) {
            ...
            DPRINTF(HWPrefetch, "Current address is %#lx,"
                "Generated prefetch is %#lx,"
                "stride is %#lx\n", addr, prefetch_addr, bestOffset);
        }
        else {
            DPRINTF(HWPrefetch, "Stop prefetch,"
                "prefetch address and"
                "demand request are not in the same page\n");
        }
    }
}
